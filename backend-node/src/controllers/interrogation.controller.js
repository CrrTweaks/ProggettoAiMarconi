...
export const list = asyncHandler(async (req, res) => {
  ...
  if (class_id) { params.push(class_id); where += ` AND i.class_id=$${params.length}`; }
  if (from)     { params.push(from);     where += ` AND i.scheduled_for >= $${params.length}`; }
  if (to)       { params.push(to);       where += ` AND i.scheduled_for <= $${params.length}`; }
...
export const create = asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    ...
    [class_id, student_id || null, req.user.id, subject || null, topic || null, scheduled_for]
  );
  if (student_id) {
    const { rows: n } = await query(
      `INSERT INTO notifications (user_id, type, title, body, data)
       VALUES ($1,'interrogation',$2,$3,$4) RETURNING *`,
      [student_id, `Interrogazione programmata: ${subject || ''}`.trim(), topic || '', { interrogation_id: rows[0].id }]
    );
    try { emitToUser(student_id, 'notification:new', n[0]); } catch {}
  }
...
export const update = asyncHandler(async (req, res) => {
  ...
  if (!rows[0]) throw new HttpError(404, 'Interrogation not found');
