...
export const list = asyncHandler(async (req, res) => {
  ...
  if (class_id) { params.push(class_id); where += ` AND h.class_id=$${params.length}`; }
  if (from)     { params.push(from);     where += ` AND h.due_date >= $${params.length}`; }
  if (to)       { params.push(to);       where += ` AND h.due_date <= $${params.length}`; }
...
export const create = asyncHandler(async (req, res) => {
  const { class_id, title, description, subject, due_date, priority } = req.body;
  const { rows } = await query(
    `INSERT INTO homework (class_id, title, description, subject, due_date, priority, assigned_by)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
    [class_id, title, description || null, subject || null, due_date, priority || 1, req.user.id]
  );
  ...
  for (const m of members) {
    const { rows: n } = await query(
      ...
      [m.user_id, `Nuovi compiti: ${title}`, description || '', { homework_id: rows[0].id }]
    );
...
export const remove = asyncHandler(async (req, res) => {
  await query('UPDATE homework SET deleted_at=NOW() WHERE id=$1', [req.params.id]);
  res.json({ ok: true });
});
