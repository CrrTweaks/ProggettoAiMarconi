...
export const list = asyncHandler(async (req, res) => {
  ...
  if (class_id) { params.push(class_id); where += ` AND a.class_id = $${params.length}`; }
...
export const create = asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    ...
    [user_id, class_id || null, date, hours || 1, justified || false, reason || null]
  );
...
export const update = asyncHandler(async (req, res) => {
  ...
  if (!rows[0]) throw new HttpError(404, 'Absence not found');
