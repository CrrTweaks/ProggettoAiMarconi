...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    upstream.data.on("error", (e) => {
      res.write(
        `data: ${JSON.stringify({ event: "error", data: e.message })}\n\n`,
      );
      res.end();
    });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!own.rowCount) throw new HttpError(404, "Chat non trovata");
    const { rows } = await query(
...
router.delete(
  "/chats/:id",
  asyncHandler(async (req, res) => {
    await query(`DELETE FROM ai_chats WHERE id=$1 AND user_id=$2`, [
      req.params.id,
      req.user.id,
    ]);
    res.json({ ok: true });
...
router.delete(
  "/rag/documents/:id",
  asyncHandler(async (req, res) => {
    await query(`DELETE FROM pdf_documents WHERE id=$1 AND user_id=$2`, [
      req.params.id,
      req.user.id,
    ]);
    res.json({ ok: true });
...
router.post(
  "/rag/query",
  asyncHandler(async (req, res) => {
    const { data } = await aiClient.post("/rag/query", {
      ...req.body,
      user_id: req.user.id,
    });
    res.json(data);
...
router.post(
  "/concept-map",
  asyncHandler(async (req, res) => {
    const { data } = await aiClient.post("/concept-map", {
      ...req.body,
      user_id: req.user.id,
    });
    res.json(data);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!rows[0]) throw new HttpError(404, "Mappa non trovata");
    res.json({ map: rows[0] });
  }),
);
...
router.post(
  "/voice/tts",
  asyncHandler(async (req, res) => {
    const upstream = await aiClient.post("/voice/tts", req.body, {
      responseType: "stream",
    });
      ...
      `WITH base AS (
       ...
       SELECT (b.start_date + d) AS day,
              ...
              (SELECT COUNT(*) FROM exams e
                 WHERE e.class_id=$1 AND e.scheduled_for::date = b.start_date + d) AS exams,
              (SELECT COUNT(*) FROM interrogations i
                 WHERE i.class_id=$1 AND i.scheduled_for::date = b.start_date + d) AS interrogations
       FROM days, base b
       ORDER BY day`,
      [class_id, week_start],
    );
